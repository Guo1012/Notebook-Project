...
def list_notebooks():
    items = []
    for notebook_id, revision, content in repository.list():
        metadata = content.get("metadata", {})
        lumen = metadata.get("lumen", {}) if isinstance(metadata, dict) else {}
        cells = content.get("cells", [])
        items.append(
            {
                "notebookId": notebook_id,
                "title": lumen.get("title", "未命名 Notebook") if isinstance(lumen, dict) else "未命名 Notebook",
                "revision": revision,
                "cellCount": len(cells) if isinstance(cells, list) else 0,
                "createdAt": lumen.get("created_at", "") if isinstance(lumen, dict) else "",
                "updatedAt": lumen.get("updated_at", "") if isinstance(lumen, dict) else "",
            }
        )
    return {"items": items}
