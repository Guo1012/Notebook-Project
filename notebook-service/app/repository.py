...
class LocalNotebookRepository:
    ...
    def list(self) -> list[tuple[str, int, dict[str, Any]]]:
        notebooks: list[tuple[str, int, dict[str, Any]]] = []
        ...
        for directory in self.data_dir.iterdir():
            if not directory.is_dir():
                continue
            try:
                revision, content = self.get(directory.name)
            except (NotebookNotFound, OSError, ValueError, json.JSONDecodeError):
                continue
            notebooks.append((directory.name, revision, content))
        return notebooks
